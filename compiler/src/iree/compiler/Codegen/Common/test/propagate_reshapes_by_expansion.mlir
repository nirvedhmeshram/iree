// RUN: iree-opt --pass-pipeline="builtin.module(func.func(iree-codegen-propagate-reshapes-by-expansion), cse)" --split-input-file %s --mlir-print-local-scope | FileCheck %s
...
func.func @no_expand_dest_forall_not_workgroup_mapped() {
  ...
  %2 = scf.forall (%arg0, %arg1) = (0, 0) to (2048, 10240) step (128, 128) 
     shared_outs(%arg2 = %1) -> (tensor<2048x10240xf32>) {
    %extracted_slice = tensor.extract_slice %arg2[%arg0, %arg1] [128, 128] [1, 1] : tensor<2048x10240xf32> to tensor<128x128xf32>
    %3 = tensor.empty() : tensor<8x8x16x8x2xf32>
    %4 = linalg.fill ins(%cst : f16) outs(%3 : tensor<8x8x16x8x2xf32>) -> tensor<8x8x16x8x2xf32>
    %5 = tensor.empty() : tensor<8x16x8x8x2xf32>
    %transposed = linalg.transpose ins(%4 : tensor<8x8x16x8x2xf32>) outs(%5 : tensor<8x16x8x8x2xf32>) permutation = [0, 2, 1, 3, 4] 
    %expanded = tensor.expand_shape %extracted_slice [[0, 1], [2, 3, 4]] output_shape [8, 16, 8, 8, 2] : tensor<128x128xf32> into tensor<8x16x8x8x2xf32>
    %6 = linalg.copy ins(%transposed : tensor<8x16x8x8x2xf32>) outs(%expanded : tensor<8x16x8x8x2xf32>) -> tensor<8x16x8x8x2xf32>
    %collapsed = tensor.collapse_shape %6 [[0, 1], [2, 3, 4]] : tensor<8x16x8x8x2xf32> into tensor<128x128xf32>
    scf.forall.in_parallel {
      tensor.parallel_insert_slice %collapsed into %arg2[%arg0, %arg1] [128, 128] [1, 1] : tensor<128x128xf32> into tensor<2048x10240xf32>
    }
  } 
  flow.dispatch.tensor.store %2, %0, offsets = [0, 0], sizes = [2048, 10240], strides = [1, 1] : tensor<2048x10240xf32> -> !flow.dispatch.tensor<writeonly:tensor<2048x10240xf32>>
  return
}
