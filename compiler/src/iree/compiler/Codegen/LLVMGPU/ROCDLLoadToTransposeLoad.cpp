...
struct TransferReadTransposeToGlobalTransposeLoad final
  ...
  LogicalResult matchAndRewrite(vector::TransposeOp transposeOp,
    ...
    std::optional<int64_t> expectedN = getGlobalTransposeLoadVectorSize(elemType);
    if (!expectedN)
      return rewriter.notifyMatchFailure(transposeOp,
    ...
    if (!inBounds ||
        !llvm::all_of(inBounds.getAsRange<BoolAttr>(),
    ...
    if (memrefType.getMemorySpace()) {
      auto globalSpace = gpu::AddressSpaceAttr::get(
          rewriter.getContext(), gpu::AddressSpace::Global);
      auto globalMemrefType = MemRefType::get(
    ...
    if (!hasSharedMemoryAddressSpace(writeDst))
      return rewriter.notifyMatchFailure(transposeOp,
                                         "write destination is not workgroup memory");
    ...
    //   N_new = N_base + K_single % N      (lane's N position within N_base group)
    //   K_new = (K_single // N) * N        (K-group base, aligned to N)
...
struct ROCDLLoadToTransposeLoadPass final
  ...
  void runOnOperation() override {
    ...
    if (isGfx950) {
      ...
      patterns.add<PropagateHintThroughDelinearize,
                   TransferReadToTransposeLoad>(funcOp.getContext());
    }
