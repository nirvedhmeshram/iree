...
#include "experimental/xrt/direct_allocator.h"
...
static iree_hal_xrt_allocator_t* iree_hal_xrt_allocator_cast(iree_hal_allocator_t* base_value) {
  IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_xrt_allocator_vtable);
  return (iree_hal_xrt_allocator_t*)base_value;
}

iree_status_t iree_hal_xrt_allocator_create(iree_hal_device_t* base_device, xrt::device device,
                                              iree_allocator_t host_allocator,
  ...
  if (iree_status_is_ok(status)) {
    iree_hal_resource_initialize(&iree_hal_xrt_allocator_vtable, &allocator->resource);
    allocator->base_device = base_device;
    iree_hal_device_retain(base_device);
    allocator->device = device;  // +1
...
static void iree_hal_xrt_allocator_destroy(iree_hal_allocator_t* IREE_RESTRICT base_allocator) {
  iree_hal_xrt_allocator_t* allocator = iree_hal_xrt_allocator_cast(base_allocator);
...
static iree_allocator_t iree_hal_xrt_allocator_host_allocator(
  ...
  iree_hal_xrt_allocator_t* allocator = (iree_hal_xrt_allocator_t*)base_allocator;
...
static iree_hal_buffer_compatibility_t iree_hal_xrt_allocator_query_buffer_compatibility(
    iree_hal_allocator_t* IREE_RESTRICT base_allocator,
...
static iree_status_t iree_hal_xrt_allocator_allocate_buffer(
  ...
  // It seems host side bank is bank 8 so thats what we will set the
  // group id as

  int group_id = 8;

  xrt::bo xrt_buffer = xrt::bo(allocator->device, allocation_size, XRT_BO_FLAGS_HOST_ONLY, group_id);
  ...
  if (iree_status_is_ok(status)) {
    status = iree_hal_xrt_buffer_wrap(
        xrt_buffer, base_allocator, compat_params.type, compat_params.access, compat_params.usage,
        allocation_size, /*byte_offset=*/0,
        /*byte_length=*/allocation_size, iree_hal_buffer_release_callback_null(), &buffer);  // +1
  }

  if (iree_status_is_ok(status)) {
    IREE_TRACE_ALLOC_NAMED(IREE_HAL_XRT_ALLOCATOR_ID, (void*)iree_hal_xrt_buffer_handle(buffer),
                           allocation_size);
...
/*.query_memory_heaps=*/iree_hal_xrt_allocator_query_memory_heaps,
    /*.query_buffer_compatibility = */iree_hal_xrt_allocator_query_buffer_compatibility,
